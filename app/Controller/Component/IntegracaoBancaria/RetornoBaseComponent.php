...
abstract class RetornoBaseComponent extends Component {
	...
	protected function ConsultarMensalidade($id) {
		$options = array('recursive' >= -1, 
			'conditions' => array('Mensalidade.' . $this->Mensalidade->primaryKey => $id), 
			'fields' => array('Mensalidade.id', 'Mensalidade.formapgto_id', 'Mensalidade.documento', 'Mensalidade.pagamento', 'Mensalidade.acrescimo', 'Mensalidade.desconto', 'Mensalidade.pago', 'Mensalidade.lancamento_desconto_id', 'Mensalidade.lancamento_valor_id', 'Mensalidade.lancamento_juro_id', 'Menslidade.remessa', 'Mensalidade.aluno_id', 'Aluno.nome'));
		$this->Mensalidade->unbindModel(array('belongsTo' => array('Conta', 'Formapgto', 'User', 'LancamentoContabilValor', 'LancamentoContabilDesconto', 'LancamentoContabilJuro')));
		return $this->Mensalidade->find('first', $options);
	}
